...
function Api_SalOrder_Save($custid)
{
	...
	"FBuyerWithholding":"false"}]}],"FSaleOrderPlan":[{"FEntryID":"0","FNeedRecAdvance":"true","FReceiveType":{"FNumber":""},"FRecAdvanceRate":"0",
 ...
 * 收款单保存调用
 */
function Api_SKD_Save($billno,$jsfs,$skje)
{
	header("Content-type:text/html;charset=utf-8");
	$cloudUrl="http://192.168.10.5:8888/K3Cloud/";
	$cookie_jar = tempnam('./tmp','CloudSession');
	...
	$Obj=$obj->Context;
	//根据客户ID获取出客户的Number
	$custid=M("t_sal_order")->where("fbillno='".$billno."'")->getField("fcustid");
	$custnumber=M("t_bd_customer")->where("fcustid=".$custid."")->getField("fnumber");
	$salerId=M("v_bd_salesman")->join("t_bd_customer on v_bd_salesman.fid=t_bd_customer.fseller")->where("t_bd_customer.fcustid=".$custid." ")->getField("v_bd_salesman.fnumber");
	$date=Date('Y-m-d',time());
	if($jsfs==0)
	{ //银行代扣
		$jsfs_number="JSFS05_SYS";
	}
	if($jsfs==1)
	{
		$jsfs_number="JSFS04_SYS";
	}
	$sal_order_id=M("t_sal_order")->where("fbillno='".$billno."'")->getField("fid");
	//检测是否生成过付款单 T_AR_RECEIVEBILL
	$icount=M("t_ar_receivebill")->where("F_PAEZ_Text='".$billno."'")->count();
	if($icount>0)
	{
		return "f";
	}else {
		...
		$skyt='SFKYT01_SYS';
		$data='{"Creator":"","NeedUpDateFields":[],"NeedReturnFields":[],"IsDeleteEntry":"True","SubSystemId":"","IsVerifyBaseDataField":"false",
	"Model":{"FID":"0","FBillTypeID":{"FNumber":"SKDLX01_SYS"},"FDATE":"'.$date.'",
	"FPAYUNITTYPE":"BD_Customer","FPAYUNIT":{"FNumber":"'.$custnumber.'"},
	"FCONTACTUNITTYPE":"BD_Customer","FCONTACTUNIT":{"FNumber":"'.$custnumber.'"},
	"FCURRENCYID":{"FNumber":"PRE001"},"FPAYORGID":{"FNumber":"102"},"FSETTLERATE":"1","FSETTLEORGID":{"FNumber":"102"},
	"FSALEORGID":{"FNumber":"102"},"FSALEERID":{"FNumber":"'.$salerId.'"},
	"FBUSINESSTYPE":"1","FISINIT":"false","FEXCHANGERATE":"1","FCancelStatus":"","FSETTLECUR":{"FNumber":"PRE001"},"FISB2C":"false","F_PAEZ_CheckBox":"false","FWBSETTLENO":"",

	"F_PAEZ_Text":"'.$billno.'",
	"F_PAEZ_CheckBox1":"false",
	"FRECEIVEBILLENTRY":[{"FEntryID":"0","FSETTLETYPEID":{"FNumber":"'.$jsfs_number.'"},"FPURPOSEID":{"FNumber":"'.$skyt.'"},"FRECEIVEITEMTYPE":"","FRECEIVEITEM":"",
	"FSaleOrderID":"'.$sal_order_id.'","FRECTOTALAMOUNTFOR":"'.$skje.'","FRECAMOUNTFOR_E":"'.$skje.'","FSETTLEDISTAMOUNTFOR":"0","FHANDLINGCHARGEFOR":"0",
	"FREALRECAMOUNTFOR_D":"'.$skje.'","FACCOUNTID":{"FNumber":"22240401040000713"},
	"FOPPOSITEBANKACCOUNT":"","FOPPOSITECCOUNTNAME":"","FINNERACCOUNTID":{"FNumber":""},"FCashAccount":{"FName":""},"FSETTLENO":"","FOPPOSITEBANKNAME":"","FCOMMENT":"'.$billno.'",
	"FRECAMOUNT_E":"'.$skje.'","FPOSTDATE":"1900-01-01","FMATERIALID":{"FNumber":""},"FSALEORDERNO":"'.$billno.'","FMATERIALSEQ":"0","FORDERENTRYID":"0" }]
	  } } ';
	//echo $data;
		//保存
		$save_data = array(
 ...
 * 检测订单状态，再次进行提交和审核
 */
function ContinueSubmit($billId)
{
    $api_msg="";
    sleep(20);
    //取出订单的状态
    $M = new \Think\Model();
    $billlist = $M->query("select fdocumentstatus,fbillno from t_sal_order where fid=" . $billId);
    $billStatus = $billlist[0]["fdocumentstatus"];
    $billno = $billlist[0]["fbillno"];
    //取出收费记录表，fssje、fzfje字段
    $sfqklist = $M->query("select fzfje,fssje from jt_orderjsrecord where billid=" . $billId);
    $ssje = $sfqklist[0]["fssje"];
    $zfje = $sfqklist[0]["fzfje"];
    if ($ssje >= $zfje && $billStatus != "C") { //实收金额大于需支付的金额且单据状态不是已审核，执行提交审核
        //1.检测有无金蝶收款单记录
        $icount = M("t_ar_receivebill")->where("F_PAEZ_Text='" . $billno . "'")->count();
        if ($icount == 0) {//如果没有收款单记录，重新调用金蝶API去保存收款单
            $res_save = Api_SKD_Save($billno, 0, $ssje);
            if ($res_save == "f") {
                $api_msg = "你好，你的订单支付已成功，但是系统在调用API保存收款时遇到问题，该订单需要联系雪宝工作人员进行线下处理！";
            }else{
                $skdno = $res_save[0]->Number;
                $shjg = Api_SKD_Audit($skdno);//利用Api提交、审核收款单
            }
        } else {//有收款单记录，则检测收款单的状态
            $skd = $M->query("select fbillno,fdocumentstatus from t_ar_receivebill where F_PAEZ_Text='" . $billno . "'");
            if ($skd[0]["fdocumentstatus"] != "C") {
                $shjg = Api_SKD_Audit($skd[0]["fbillno"]);//利用Api提交、审核收款单
            }
        }
        //2.提交审核销售订单
