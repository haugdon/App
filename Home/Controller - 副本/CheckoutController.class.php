...
class CheckoutController extends CommonController {
    public function index() {
        ...
        */
        ...
        } else if (substr($cust_name, 0, 9) == "市场部" || $_GET['isbox'] == "1") { //或者$is_box = I('get.isbox', '');//这两张方式都可以获取到isbox
            $def_shtime = date("Y-m-d");
        } else {
		...
		$this->assign("custid", session("custid2"));
        $this->assign("def_shtime", $def_shtime);
        $this->getfanlirule();
    ...
    public function saveOrder() {
        ...
        //调用API进行单据头保存，以获取单据编号，避免单据重复
        $res_save = Api_SalOrder_Save($custid);
        if ($res_save == "f") {
            echo json_encode(array("success" => false, "error" => urldecode("调用API单据保存失败！你可以尝试重新提交或联系管理员解决!")));
            die();
        }
        $API_BILLID = $res_save[0]->Id;
        if (empty($API_BILLID)) {
            echo json_encode(array("success" => false, "error" => urldecode("API_BILLID为空,调用单据保存失败！你可以尝试重新提交或联系管理员解决!")));
            die();
        }
        //更新单据头
        $Res = $M->query("exec Jp_update_SalOrder " . $API_BILLID . "," . $creatorId . "," . $shyid . "," . $custid . "," . $custjhadd_id . "");
        if ($Res) {
            foreach ($Res as $row) {
                $FID = $row["fid"];
            ...
            if ($R) {
                echo json_encode(array("success" => true, "msg" => urldecode("订单提交成功！"), "FID" => $FID, "fhrq" => $fhtime));
            } else {
