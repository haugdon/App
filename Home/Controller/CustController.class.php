...
class CustController extends CommonController {
     ...
     * add by kevin holden 20180129
     */
    public function showtkmx() {
        $this->gettkmxlist();
        $this->display("Cust/tkmx");
    }
    ...
    /**
     * 退款明细列表查询
     * add by kevin holden 20180504
     */
    function gettkmxlist() {
        //        $status = I("status", 'dfk');
        $custid = session("custid");
        $M = new \Think\Model();
        $list = $M->query("exec Jp_gettkmxlist " . $custid . "");
        $this->assign("tkmxlist", $list);
    }
    ...
    function getorderlist() {
        ...
        if ($status == 'dfh') {
            foreach ($list as $n => $val) {
                $list[$n]['childList'] = $ChildM->query("exec Jp_getorderlist_detail_dfh " . $val["fid"] . " ");/*modify by kevin 给待发货单独创建了存储过程,避免了待收货和已完成被影响*/
            }
        } else {
            foreach ($list as $n => $val) {
    ...
    /**
     * add by kevin 用于调试输出到chrome console
     */
