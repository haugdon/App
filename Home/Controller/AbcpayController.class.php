...
class AbcpayController extends CommonController {//这个文件的编码格式一定要是GBK
    /**
     * 显示农行代扣支付页面
     */
    ...
    public function abcpay() {
        ...
        if ($fkaccount == "") {
            echo json_encode(array("jyjg" => mb_convert_encoding("支付失败[在经销商订货平台AbcpayController第54行]", "UTF-8", "GBK"), "msg" => mb_convert_encoding("付款账号不能为空！", "UTF-8", "GBK")));
            exit();
        }
    ...
    function abc_payment_send($fkaccount, $amount, $billno, $orderid, $fkaccname) {
        ...
        */
        ...
        if ($array["RespSource"] == 0) {
            ...
            return array("jyjg" => mb_convert_encoding("支付失败[在经销商订货平台AbcpayController中第121行]", "UTF-8", "GBK"), "msg" => $array["RespInfo"] . $array["RxtInfo"]);//mb_convert_encoding($R,"UTF-8","GBK"));
        }
    }
