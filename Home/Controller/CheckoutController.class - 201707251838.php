...
class CheckoutController extends CommonController {
    ...
    public function getcusaddr() {
        $custid = session("custid");
        $M = new \Think\Model();
        $list = $M->query("select a.fcustid,fname,FADDRESS,b.fcontact,a.fentryid,a.fisdefaultconsignee as isdef from T_BD_CUSTLOCATION a left join T_BD_CUSTCONTACT b on a.FTCONTACT=b.FCONTACTID  where a.FCUSTID=" . $custid . " order by FISDEFAULTCONSIGNEE desc ");
    ...
    public function saveOrder() {
        ...
        if ($dkfs == 2) {
            ...
            if ($pxlist) {
                ...
                if ($pxchkmess <> "") {
                    echo json_encode(array("error" => $pxchkmess));
                    exit();
                }
            }
        }
        ...
        if (!$plist) {
            echo json_encode(array("error" => urldecode("支付密码不正确!")));
            exit();
        }
        //余额抵扣方式
        ...
        if ($Res) {
            ...
            if ($R) {
                //生成网上订单
                //  $M->execute("exec Jp_set_Cporder_auto ".$FID."");
