...
class ShyController extends CommonController {
    public function index() {
        ...
        $this->showdaisonghuolist();//mark by hd
        $this->showoverlist();
        $this->display();
    }
    ...
    public function showdaisonghuolist() {
        $custid = session("custid");
        $this->console_log("custid=" . $custid);
        $M = new \Think\Model();
        $mlist = $M->query("select b.fid from t_bd_customer_l a,t_hr_empinfo_l b where a.fname=b.fname and a.fcustid=" . $custid . "");
        $this->console_log($mlist);
        foreach ($mlist as $r) {
            $shyid = $r["fid"];
        }
        ...
        //        echo 'foreach循环了' . $i . '次';//不用这种土方式,这样会直接打印到render的页面上,用我自定义的方式打印到浏览器控制台
        $this->console_log('获取当前已抢单待送货单据方法中的Jp_getorderlist_shy_dsh的foreach循环了' . $i . '次');
    ...
    public function showoverlist() {
        ...
        $this->console_log('获取当前已送货完成订单方法中的Jp_getorderlist_shy_over的foreach循环了' . $i . '次');
    ...
    public function searchorder() {
        $custid = session("custid");
        $M = new \Think\Model();
        $mlist = $M->query("select b.fid from t_bd_customer_l a,(select aa.fid,aa.FNAME from t_hr_empinfo_l aa left join t_hr_empinfo bb on aa.fid=bb.fid where bb.FFORBIDSTATUS='A' ) b where a.fname=b.fname and a.fcustid=" . $custid . "");
        foreach ($mlist as $r) {
            $shyid = $r["fid"];
        }
        if ($shyid == null) {
    ...
    public function saveorder() {
        ...
        if ($total_amount > 0) //如果金额大于0，则跳转到结算支付页面
        {
            ...
            for ($i = 0; $i < $count; $i++) //判断是否金额大于0
            {
                $sql = "exec Jp_setBasket_shy " . $materialid[$i] . "," . $qty[$i] . "," . $userid . ",0," . $dj[$i] . "," . $randid . "," . $boxmid[$i] . ",1 ";
                $res = $M->execute($sql);
            }
            if ($res) {
    ...
    /**
     * add by hd 用于调试输出到chrome console
     */
